...
{error && <Alert variant="danger">{error}</Alert>}
  <h4>Articles / Stories</h4>
      <Button style={{margin:"1rem"}} onClick={() => setLgShow(true)} className="me-2">Add Article/Story</Button>
      <Modal
        size="lg"
        show={lgShow}
      ...
      {experienceList?(experienceList.map((exp,id) => {
        return (
          ...
          <div className="card_content">
            <h3>{exp.title}</h3>
            {exp.desc.length>200?(<p style={{textAlign:"left"}}>{exp.desc.substring(0, 200)}...</p>):(<p>{exp.desc}</p>)}
            <Link to={`/exp/${id}`}><Button variant="primary">Read more</Button></Link>
            </div>
            </div>
          </Card.Body>
        </Card>
